...
# -------------------------remove from heap--------------------------
    ...
    def checkMaxHeap(self, data, i):
        left = 2*i + 1
        right = 2 *i +2

        if left > len(data) or right > len(data):
            return True

        left = data[i] > data[left] and self.checkMaxHeap(data, left)
        right = data[i] > data[right] and self.checkMaxHeap(data, right)
